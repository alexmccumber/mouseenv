...
metaDF$SoilType = factor(metaDF$SoilType, levels = c("Control", "Pine", "River", "Road"))

metaDF$Virus = factor(metaDF$Virus, levels = c("HI", "PR8"))
```
...
#saveRDS(ddsDE, "~/mouseenv/ddsDE.RDS")
...
ResSoilRoad = results(ddsDE, name = "SoilTypeRoad.VirusPR8", tidy = T)
...
p=plotCounts(ddsDE, gene="ENSMUSG00000025492", intgroup=c("SoilType"), returnData =T)

p$ID = rownames(p)

p=merge(p, MouseLungMetaData, by = "ID")

r =  ggplot(p, aes(y = count, x = SoilType, color = SoilType, shape = Flu)) +
  geom_point(aes(size = 2, alpha = 0.8)) +
  theme_minimal()+ 
  theme(legend.position = "none") +
  ylab("IFITM3 DESeq2 Normalized Counts") + 
  xlab(NULL)

p=plotCounts(ddsDE, gene="ENSMUSG00000030748", intgroup=c("SoilType"), returnData =T)

p$ID = rownames(p)

p=merge(p, MouseLungMetaData, by = "ID")

f=ggplot(p, aes(y = count, x = SoilType, color = SoilType, shape = Flu)) +
  geom_point(aes(size = 2, alpha = 0.8)) +
  theme_minimal()+ 
  theme(legend.position = "none") +
  ylab("IL4A DESeq2 Normalized Counts") + 
  xlab(NULL)

p=plotCounts(ddsDE, gene="ENSMUSG00000032089", intgroup=c("SoilType"), returnData =T)

p$ID = rownames(p)

p=merge(p, MouseLungMetaData, by = "ID")

g=ggplot(p, aes(y = count, x = SoilType, color = SoilType, shape = Flu)) +
  geom_point(aes(size = 2, alpha = 0.8)) +
  theme_minimal()+ 
  theme(legend.position = "none") +
  ylab("IL10RA DESeq2 Normalized Counts") + 
  xlab(NULL)

p=plotCounts(ddsDE, gene="ENSMUSG00000004371", intgroup=c("SoilType"), returnData =T)

p$ID = rownames(p)

p=merge(p, MouseLungMetaData, by = "ID")

q=ggplot(p, aes(y = count, x = SoilType, color = SoilType, shape = Flu)) +
  geom_point(aes(size = 2, alpha = 0.8)) +
  theme_minimal()+ 
  theme(legend.position = "none") +
  ylab("IL11 DESeq2 Normalized Counts") + 
  xlab(NULL)

x = ggarrange(r,f,g,q,  labels = c("A", "B", "C", "D"), ncol = 2, nrow = 2)
```
...
Delta_DGE = rbind(ResSoilPine, ResSoilRiver)
write.csv(Delta_DGE, "~/mouseenv/RNASeqRes/soilXvirusDelta/DeltaSoil_DGE.csv")
...
write.csv(ResSoilPR8, "~/mouseenv/RNASeqRes/soilXvirusDelta/PR8vHI_DGE.csv")
```
...
```{r, no GO terms or KEGG pathways for Pine samples, need not run}
...
write.csv(fc.kegg.sigmet.p.up, "~/mouseenv/RNASeqRes/soilXvirusDelta/Delta_River_KEGG.csv")
...
write.csv(PR8_River_GOterm, "~/mouseenv/RNASeqRes/soilXvirusDelta/Delta_River_GOterm.csv")
```
...
#Need to make a better plot here
plotCounts(ddsDE, gene="ENSMUSG00000025492", intgroup=c("SoilType"))
```
...
write.csv(HI_DGE, "~/mouseenv/RNASeqRes/HIBaseline/HI_DGE.csv")
```
...
#results for Soil specific, 407 genes
ResSoilPine = results(ddsDE, name = "SoilType_Pine_vs_Control", tidy = T) %>%
  dplyr::filter(., padj < 0.1) %>%
  dplyr::filter(., log2FoldChange >=1 | log2FoldChange <= -1)

#results for Soil specific, 1884 genes
ResSoilRiver = results(ddsDE, name = "SoilType_River_vs_Control", tidy = T) %>%
  dplyr::filter(., padj < 0.1) %>%
  dplyr::filter(., log2FoldChange >=1 | log2FoldChange <= -1)

#results for Soil specific, 28 genes
ResSoilRoad = results(ddsDE, name = "SoilType_Road_vs_Control", tidy = T) %>%
  dplyr::filter(., padj < 0.1) %>%
  dplyr::filter(., log2FoldChange >=1 | log2FoldChange <= -1)
```
...
write.csv(PR8_DGE, "~/mouseenv/RNASeqRes/PR8Baseline/PR8_DGE.csv")
```
...
write.csv(fc.kegg.dise.p.up, "~/mouseenv/RNASeqRes/PR8Baseline/PR8_Pine_KEGG.csv")
...
write.csv(PR8_Pine_GOterm, "~/mouseenv/RNASeqRes/PR8Baseline/PR8_Pine_GOterm.csv")
```
...
write.csv(fc.kegg.dise.p.up, "~/mouseenv/RNASeqRes/PR8Baseline/PR8_River_KEGG.csv")
...
write.csv(PR8_River_GOterm, "~/mouseenv/RNASeqRes/PR8Baseline/PR8_River_GOterm.csv")
```
